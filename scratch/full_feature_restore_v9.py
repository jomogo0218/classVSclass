...
def full_feature_restore_v9():
    print("Executing Full Feature Restore V9 (Merging Original Logic + New Tournament Dashboard)...")
    ...
    for cat, teams in groups.items():
        ...
        for idx, (tA, tB) in enumerate(pairs):
            m = {"id": f"{cat}_{idx}", "category": cat, "teamA": tA, "teamB": tB, "status": "⏳ 待打", "score": "0:0", "winner": None}
            for rc, ra, rb, rs, rw in results:
                if rc == cat and ((ra in tA and rb in tB) or (ra in tB and rb in tA)):
                    m.update({"status": "✅ 已結束", "score": rs, "winner": rw, "scoreA": int(rs.split(':')[0]), "scoreB": int(rs.split(':')[1])})
                    break
            final_matches.append(m)
    
    # 2. 獲取原始素材 (HTML / CSS / JS / Schedules)
    html_raw = open('d:/classVSclass/index.html', 'r', encoding='utf-8').read()
    css_raw = open('d:/classVSclass/app.css', 'r', encoding='utf-8').read()
    js_raw = open('d:/classVSclass/app.js', 'r', encoding='utf-8').read()
...
.rr-matrix-cell { width: 80px; height: 40px; background: #f8fafc; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 13px; }
.rr-matrix-cell.win { background: #10b981; color: white; }
.category-tab { padding: 14px 20px; cursor: pointer; display: flex; align-items: center; gap: 10px; font-weight: 700; border-bottom: 1px solid #eee; }
.category-tab.active { background: #fff7ed; color: #f5750a; border-right: 5px solid #f5750a; }
#loadingOverlay { transition: opacity 0.3s; }
"""

    # 4. 混合 JS (關鍵：保留原始邏輯 + 注入新邏輯)
    js_mix = """
window.EMBEDDED_MATCHES = """ + json.dumps(final_matches, ensure_ascii=False) + """;
window.EMBEDDED_SCHEDULES = """ + schedules_json + """;

// Fetch 攔截器 (確保原始 app.js 抓到正確資料)
(function() {
...
const originalSwitchTab = window.switchTab || function(tabId) {
...
window.switchTab = function(tabId) {
    ...
    if(tabId === 'tournament') renderTournamentOverview();
};
    ...
    c.innerHTML = `
        <div class="rr-dashboard">
            ...
            <div class="rr-leaderboard">
                <div style="font-weight:800; margin-bottom:15px;">🏆 積分排行榜</div>
                ${Object.entries(stats).sort((a,b)=>b[1].pts-a[1].pts || b[1].net-a[1].net).map(([t,s], i) => `
                    <div class="rr-rank-card"><div style="width:30px; font-weight:900;">#${i+1}</div><div style="flex:1; font-weight:700; font-size:13px;">${t}</div><div style="text-align:center;"><div style="color:#f5750a; font-weight:900;">${s.pts}</div><div style="font-size:9px;">PTS</div></div><div style="width:50px; text-align:right; font-weight:800; color:${s.net>=0?'#10b981':'#ef4444'}">${s.net>0?'+':''}${s.net}</div></div>
                `).join('')}
            </div>
            <div class="rr-matrix-container">
                <table class="rr-matrix-table">
                    <tr><th></th>${teams.map(t=>`<th>${t}</th>`).join('')}</tr>
                    ${teams.map(rt => `<tr><td>${rt}</td>${teams.map(ct => {
                        ...
                        return `<td class="rr-matrix-cell ${(m&&m.winner===rt)?'win':''}">${s||'待打'}</td>`;
                    }).join('')}</tr>`).join('')}
                </table>
            </div>
        </div>
...
"""
    ...
    # 5. 整合最終 HTML (Aggressive Cleanup + Re-Injection)
    clean_html = re.sub(r'<script\b[^>]*src=["\']app\.js[^>]*>\s*</script>', '', html_raw, flags=re.I)
    ...
    final_output = clean_html.replace('</head>', f'<style>{css_final}</style></head>').replace('</body>', f'<script>{js_mix}</script></body>')
    
    with open('d:/classVSclass/index_local.html', 'w', encoding='utf-8') as f:
        f.write(final_output)
    
    print("V9 FULL FEATURE RESTORE COMPLETE! Both Scheduling and Tournament are active.")

if __name__ == "__main__":
    full_feature_restore_v9()
