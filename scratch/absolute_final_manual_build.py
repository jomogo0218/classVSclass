...
def final_manual_build():
    print("Starting Absolute Final Manual Build...")
...
"""
    ...
    final_html = html_raw
    # 注入 CSS
    final_html = final_html.replace('</head>', f'<style>\\n{css_final}\\n</style>\\n</head>')
    # 移除原本的 CSS 標籤 (避免干擾)
    final_html = final_html.replace('<link rel="stylesheet" href="app.css?v=20260421-3">', '')
    
    # 注入 JS
    js_block = f'<script>\\n{js_header}\\n{js_final}\\n</script>'
    # 移除所有外部 JS 標籤
    import re
    final_html = re.sub(r'<script.*src=["\\'].*?app\.js.*?["\\'].*?></script>', '', final_html)
    final_html = re.sub(r'<script.*src=["\\'].*?firebase.*?["\\'].*?></script>', '', final_html)
    ...
    print("DONE! index_local.html is finally correct.")
