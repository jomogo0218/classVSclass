...
def final_mega_integration():
    print("Executing Final Mega Integration (Data + Logic + UI)...")
    
    # 1. 準備數據 (包含 Excel 錄入的比數)
...
function renderTournamentOverview() {{
    ...
    const cats = Object.keys(grouped).sort((a,b) => (order.indexOf(a.split(' ')[0]) || 99) - (order.indexOf(b.split(' ')[0]) || 99));
            ...
            <div class="rr-matrix-container">
                <table class="rr-matrix-table">
                    ...
                    ${{teams.map(rt => `<tr><td class="rr-matrix-row-label">${{rt}}</td>${{teams.map(ct => {{
                        ...
                        return `<td class="rr-matrix-cell ${{s?'win':''}}">${{s||'0:0'}}</td>`;
                    }}).join('')}}</tr>`).join('')}}
                </table>
            </div>
        </div>
...
"""
    ...
    final_output = final_output.replace('</body>', f'<script>{js_final}</script></body>')
    
    with open('d:/classVSclass/index_local.html', 'w', encoding='utf-8') as f:
        f.write(final_output)
    
    print("ULTIMATE MEGA INTEGRATION COMPLETE! index_local.html is now perfect.")

if __name__ == "__main__":
    final_mega_integration()
