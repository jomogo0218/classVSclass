...
def final_perfect_v6_build():
    print("Executing Final Perfect V6 Build (Clean Slate Implementation)...")
...
(function() {
    ...
    window.fetch = function(url) {
        if (url.toString().includes('matches.json')) return Promise.resolve({ ok: true, json: () => Promise.resolve(window.EMBEDDED_MATCHES) });
        if (url.toString().includes('schedules.json')) return Promise.resolve({ ok: true, json: () => Promise.resolve(window.EMBEDDED_SCHEDULES) });
...
function renderTournamentOverview() {
    console.log('Rendering Overview for:', currentOverviewCategory);
    ...
    if (currentOverviewCategory) {
        container.innerHTML = '';
        const currentMatches = grouped[currentOverviewCategory] || [];
        if (currentOverviewCategory.includes('羽球')) {
            renderBracketTree(currentMatches, container);
        } else {
            renderRoundRobinDashboard(currentMatches, container);
        }
    }
}
...
function renderBracketTree(ms, c) {
    c.innerHTML = `<h3 style="color:#f5750a; padding:20px;">${currentOverviewCategory} 晉級圖</h3>`;
...
"""
    ...
    print("ULTIMATE V6 BUILD COMPLETE! All matrices are full and functional.")

if __name__ == "__main__":
    final_perfect_v6_build()
